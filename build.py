...
<div id="app">
  <header>
    ...
    <div class="hdr-right">
      ...
      <button class="btn-icon" id="btn-padron" style="display:none" onclick="refreshPadron()" title="Recargar POS desde Google Sheets">🔄 Padrón</button>
  ...
  <div id="modal">
    ...
    <div class="modal-body">
      ...
      <div id="seccion-abierto" style="display:none">
        ...
        <hr class="section-divider">
        <div class="section-title">▸ Stock Velo</div>
        <div class="field-group">
          <label class="field-label">¿La tienda tiene stock de Velo Menta Fresca? Detalla las cantidades.</label>
          <textarea id="m-stock-menta" rows="2" placeholder="Ej: 2 unidades, media caja, sin stock..."></textarea>
        </div>
        <div class="field-group">
          <label class="field-label">¿La tienda tiene stock de Velo Uva? Detalla las cantidades.</label>
          <textarea id="m-stock-uva" rows="2" placeholder="Ej: 3 unidades, stock completo..."></textarea>
        </div>
        <div class="field-group">
          <label class="field-label">¿La tienda tiene stock de Velo Sandía? Detalla las cantidades.</label>
          <textarea id="m-stock-sandia" rows="2" placeholder="Ej: sin stock, 1 caja..."></textarea>
        </div>
        <div class="field-group">
...
<!-- LOADING SCREEN -->
<div id="loading-screen" style="position:fixed;inset:0;background:linear-gradient(135deg,#1A2E6A 0%,#0f1d47 100%);display:none;align-items:center;justify-content:center;z-index:9998;flex-direction:column;gap:14px">
  <div class="ls-spinner"></div>
  <div style="color:#fff;font-size:15px;font-weight:700">Cargando puntos de ruta...</div>
  <div style="color:rgba(255,255,255,.6);font-size:11px">Conectando con Google Sheets</div>
</div>
<style>
.ls-spinner{width:42px;height:42px;border:4px solid rgba(255,255,255,.25);border-top-color:#F5A123;border-radius:50%;animation:ls-spin .75s linear infinite}
...
<script>
// ═══════════════════════════════════════════════════════════
// FALLBACK POS DATA — 90 registros (respaldo offline)
// Se reemplaza con datos del Sheet "Padrón Rutas" al hacer login
// ═══════════════════════════════════════════════════════════
{POS_JS}
let POS_DATA = FALLBACK_POS_DATA; // actualizado dinámicamente desde Google Sheets
...
let mAbierto=null, mCigarrera=null, mCigCom=null, mDispenser=null, mContrabando=null;
let fotosData={{}}, currentPosId=null;

// ─── POS DATA LOADER ───
async function loadPOSData(force){{
  // 1) Caché de sesión (saltar si force=true)
  if(!force){{
    try{{
      const cached=sessionStorage.getItem('bat_pos_data');
      if(cached){{ POS_DATA=JSON.parse(cached); console.log('POS: desde caché sesión'); return 'cache'; }}
    }}catch(e){{}}
  }}
  // 2) Intentar cargar desde Google Sheets (Padrón Rutas)
  try{{
    console.log('POS: consultando Sheet...');
    const resp=await fetch(APPS_SCRIPT_URL+'?action=getPOS',{{mode:'cors',cache:'no-cache'}});
    const json=await resp.json();
    console.log('POS Sheet response:', JSON.stringify(json).substring(0,200));
    if(json.ok&&json.data&&Object.keys(json.data).length>0){{
      POS_DATA=json.data;
      const total=Object.values(POS_DATA).reduce((s,a)=>s+a.length,0);
      try{{sessionStorage.setItem('bat_pos_data',JSON.stringify(POS_DATA));}}catch(e){{}}
      console.log('POS: '+total+' puntos cargados desde Sheet');
      return 'sheet';
    }} else {{
      console.log('POS Sheet: respuesta vacía o error:', json);
    }}
  }}catch(e){{console.warn('POS fetch error:',e.message);}}
  // 3) Fallback: datos hardcodeados en la app
  POS_DATA=FALLBACK_POS_DATA;
  console.log('POS: usando fallback hardcodeado');
  return 'fallback';
}}
...
async function refreshPadron(){{
  ...
  try{{
    sessionStorage.removeItem('bat_pos_data');
    const src=await loadPOSData(true);
    const total=Object.values(POS_DATA).reduce((s,a)=>s+a.length,0);
    try{{ buildDayTabs(); selectDay(currentDay); }}catch(err){{ console.error('refresh render error:',err); }}
    if(src==='sheet'){{ toast('✓ Padrón actualizado: '+total+' POS desde Sheets'); }}
    else{{ toast('⚠️ No se pudo conectar con Sheets — usando datos locales'); }}
  }}catch(e){{
    toast('❌ Error al actualizar padrón: '+e.message);
  }}finally{{
    btn.disabled=false; btn.textContent='🔄 Padrón';
  }}
}}
...
async function doLogin(){{
  ...
  // Mostrar pantalla de carga mientras se obtienen los POS del Sheet
  ...
  if(currentRole==='admin'){{
    ...
    document.getElementById('btn-padron').style.display='';
  }}
  try{{ initApp(); }}catch(err){{ console.error('initApp error:',err); }}
...
function doLogout(){{
  ...
  document.getElementById('btn-padron').style.display='none';
...
function openModal(posId){{
  ...
  if(ex){{
    ...
    if(ex.stockMenta) document.getElementById('m-stock-menta').value=ex.stockMenta;
    if(ex.stockUva) document.getElementById('m-stock-uva').value=ex.stockUva;
    if(ex.stockSandia) document.getElementById('m-stock-sandia').value=ex.stockSandia;
...
function resetModal(){{
  mAbierto=null;mCigarrera=null;mCigCom=null;mDispenser=null;mContrabando=null;fotosData={{}};
  ...
  ['m-staff','obs-placa','obs-vuse','obs-lucky','obs-velo','m-permite-cambio','obs-dispenser','m-stock-menta','m-stock-uva','m-stock-sandia','m-marcas-contrabando']
...
async function saveVisit(){{
  if(!mAbierto){{ toast('⚠️ Indica si el local está abierto'); return; }}
  if(!currentPosId){{ toast('⚠️ Error: sin POS seleccionado'); return; }}
  const btn=document.getElementById('btn-save');
  btn.disabled=true; btn.textContent='Guardando...';
  try{{
    const pos=getAllPos().find(p=>p.id===currentPosId);
    if(!pos) throw new Error('POS no encontrado: '+currentPosId);
    const popData={{}};
    ['placa','vuse','lucky','velo'].forEach(k=>{{
      popData[k]={{
        enc:!!document.getElementById(`pop-${{k}}-enc`)?.checked,
        opt:!!document.getElementById(`pop-${{k}}-opt`)?.checked,
        rep:!!document.getElementById(`pop-${{k}}-rep`)?.checked
      }};
    }});
    ...
    const record={{
      ...
      auditor:currentUser, dia:currentDay,
      nombre:pos.nombre, direccion:pos.direccion, distrito:pos.distrito,
      abierto:mAbierto, estado:mAbierto==='Si'?'visitado':'nogo',
      staff:gv('m-staff'),
      pop:mAbierto==='Si'?popData:{{}},
      obsPlaca:gv('obs-placa'), obsVuse:gv('obs-vuse'),
      obsLucky:gv('obs-lucky'), obsVelo:gv('obs-velo'),
      cigarrera:mAbierto==='Si'?mCigarrera:'',
      cigCom:mAbierto==='Si'&&mCigarrera==='Si'?mCigCom:'',
      permiteCambio:gv('m-permite-cambio'),
      dispenser:mAbierto==='Si'?mDispenser:'',
      obsDispenser:gv('obs-dispenser'),
      stockMenta:gv('m-stock-menta'),
      stockUva:gv('m-stock-uva'),
      stockSandia:gv('m-stock-sandia'),
      contrabando:mAbierto==='Si'?mContrabando:'',
      marcasContrabando:gv('m-marcas-contrabando'),
...
async function postToSheet(record){{
  // Enviar como text/plain para evitar CORS preflight con Apps Script
...
async function trySync(record){{
  if(!APPS_SCRIPT_URL||APPS_SCRIPT_URL.includes('TU_APPS')) return;
  try{{
...
async function syncPending(){{
  ...
  toast(`Sincronizando ${{pending.length}} visita(s)...`);
  let ok=0;
  ...
  localStorage.setItem('bat_v2_'+currentUser,JSON.stringify(visits));
  updateSyncBtn();
  toast(ok===pending.length?`✓ ${{ok}} sincronizados`:`⚠️ ${{ok}}/${{pending.length}} sincronizados`);
}}
...
async function renderDash(){{
  ...
  const conMenta=records.filter(r=>String(r.stockMenta||'').trim()).length;
  const conUva=records.filter(r=>String(r.stockUva||'').trim()).length;
  const conSandia=records.filter(r=>String(r.stockSandia||'').trim()).length;
  const conVelo=records.filter(r=>String(r.stockMenta||'').trim()||String(r.stockUva||'').trim()||String(r.stockSandia||'').trim()).length;
  ...
  <div class="dash-grid">
    ...
    <div class="kpi-card"><div class="kpi-val ok" style="font-size:26px">${{conVelo}}</div><div class="kpi-label">🟢 PDVs con Stock Velo</div></div>
  </div>
  <div class="dash-row">
    ...
    <div class="chart-card"><h3>Stock Velo por Variedad</h3><canvas id="ch-stk"></canvas></div>
  </div>
  ...
  _charts.push(new Chart(document.getElementById('ch-stk'),{{type:'bar',data:{{labels:['Menta Fresca','Uva','Sandía'],datasets:[{{label:'PDVs con stock',data:[conMenta,conUva,conSandia],backgroundColor:['#22c55e','#a855f7','#f97316']}}]}},options:{{...opt,scales:{{y:{{beginAtZero:true,ticks:{{stepSize:1}}}}}}}}}}));
  ...
  document.getElementById('gal-tabs').innerHTML=GK.map(k=>`<button class="gal-tab${{k==='fachada'?' active':''}}" onclick="switchGal('${{k}}','${{GL[k].replace(/'/g,'\\x27')}}')">${{GL[k]}}</button>`).join('');
  switchGal('fachada','Fachada');
}}

// Convierte URL de Drive viewer a URL directa embebible en <img>
function driveToImg(url){{
  if(!url) return '';
  if(url.startsWith('data:')) return url; // ya es base64, ok
...
window.switchGal=function(key,label){{
  ...
  grid.innerHTML=photos.length
    ? photos.map(r=>{{
        const src=driveToImg(r.fotos[key]);
        return `<div class="gal-item"><img src="${{src}}" loading="lazy" onerror="this.style.display='none';this.nextSibling.style.display='flex'"><div class="gal-err" style="display:none;align-items:center;justify-content:center;position:absolute;inset:0;font-size:10px;color:var(--muted);flex-direction:column;gap:2px"><span style="font-size:20px">🔒</span><span>Sin acceso</span></div><div class="gal-lbl">${{r.nombre.substring(0,18)}}</div></div>`;
      }}).join('')
